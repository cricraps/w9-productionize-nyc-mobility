# Part 1: Productionize It — NYC Mobility Pipeline

> Week 9. Takes the Week 8 pipeline in [`nyc-mobility-pipeline`](https://github.com/cricraps/nyc-mobility-pipeline) and hardens it using the Pareto practices from Day 9: version & deploy, orchestrate, recover, monitor, govern. This repo, [`w9-productionize-nyc-mobility`](https://github.com/cricraps/w9-productionize-nyc-mobility), is the result.

This document answers the Part 1 requirements directly and states what changed to get there. For architecture, the data model, and how to run the full pipeline end to end, see the main [`README.md`](../README.md); this file is the production layer on top of it.
...
| What improved from the previous repo | [§6](#6-what-changed-since-week-8) |
...
3. **Commit** — small, scoped commits (see the traffic advisory example: `Add daily job for traffic advisory`, `Document traffic advisory monitoring and recovery`, `Fix strict check count in validation comment`).
...
This mirrors the pattern documented for the traffic advisory job (PR #4, `testing` ← feature branch, commits `ad7edb1`, `8a8b9be`, `b524307`) and is used the same way for every dataset.
...
**Ownership** — per dataset, matching the main README's [Who owns what](../README.md#7-collaboration):
...
**Open governance items carried into next week:**
- The Engineer 4 decisions tab still lists two checks (`dates in window`, `segments split`) not present in the running validation notebook — needs reconciling.
- The traffic advisory job currently runs under a personal user identity; the permissions table above assumes a pipeline service identity. Needs a decision with Crizza.
- A strict-check-count comment mismatch (six vs. seven) is fixed in both the workspace and repo copies — watch that the next export doesn't undo it.

---

## 6. What changed since Week 8

Everything below is new work on top of [`nyc-mobility-pipeline`](https://github.com/cricraps/nyc-mobility-pipeline) (Week 8), carried into this repo, [`w9-productionize-nyc-mobility`](https://github.com/cricraps/w9-productionize-nyc-mobility) (Week 9). Week 8 proved the pipeline could run once, end to end; Week 9's changes are what make it safe to run again, on a schedule, unattended.

| Dataset | Week 8 (`nyc-mobility-pipeline`) | Week 9 (`w9-productionize-nyc-mobility`) |
|---|---|---|
| **Green taxi (raw)** | Months hardcoded in the notebook | Months read from a parameter box (`2026-03,2026-04,2026-05`); empty box stops the run with a clear error instead of guessing |
| | No handling for a missing month's file | Checks existence first, skips missing months with a message, doesn't crash the run |
| | Reload behavior unclear / no dedup guarantee | New rows only: incoming rows anti-joined against existing on vendor, pickup/dropoff time, location, distance, and fare (trips have no native ID) |
| | No load-time validation | Row count assertion — zero rows stops the load with an error instead of writing a broken table |
| | No proof of idempotency | Explicit idempotency check: reload May a second time on purpose, row count before/after must match exactly |
| **Taxi zones (raw)** | File picked via `files[0]` — whatever the OS listed first | Filters to known extensions, requires exactly one match — a silent wrong-file bug becomes a loud error |
| | No schema or row-count check | Row count + schema assertion before write |
| | Dedup unclear | Explicit dedup on `LocationID`, using its real primary key |
| | No idempotency proof | Pre/post row-count check confirms `overwrite` mode behaves idempotently |
| **Taxi zones (clean)** | Query ran with no filtering — a failed `CAST` silently returned `NULL`, blanks passed straight through | Cell 1 validates the source isn't empty/missing; Cell 2 explicitly hard-excludes failed casts and blank borough/zone — new logic, not a refinement |
| | Casing handled ad hoc | Cell 3 casing normalization kept as-is (EWR, SoHo, LaGuardia, JFK, DUMBO exceptions were already correct — confirmed a blanket `INITCAP` would have broken them) |
| | No write-time validation | Cell 4: row count parity, plus uniqueness/non-null checks on `location_id`, the column every downstream join depends on |
| **Weather (raw)** | Full `overwrite` every run | Delta `MERGE` keyed on `observation_time + latitude + longitude` — matches update in place, non-matches insert, so reruns don't duplicate |
| | No incremental logic | Looks up `MAX(observation_time)` already in the table, only merges source rows past that watermark (first run still writes everything) |
| | No dedup on the source itself | `dropDuplicates` on the natural key guards against overlapping timestamps in the source JSON |
| | No load-time validation | Row count assertion after write/merge (raises if 0), plus an earlier guard that raises before any write if the parsed JSON was empty |
| **Traffic advisory** | **Did not exist as a source in Week 8** | **New end-to-end source**: raw scrape (HTML, no archive) → clean Type 2 history → 10-check validation, a scheduled Databricks job with retries and failure alerting, and full recover/monitor/govern documentation (this file, §3–§5) |
| **Repo / process** | Single `main` branch, small commit history (23 commits) | Feature branch per dataset-and-layer, a shared `testing` integration branch, PR-to-`main` from feature branches (never from `testing`), and a job definition versioned in the repo (`resources/traffic_advisory_daily.job.yml`) instead of click-configured |

**The pattern underneath every row above:** Week 8 answered *"does it run?"*. Week 9 answers *"does it still work correctly if it's run again — accidentally, on a schedule, or after a partial failure?"* — every dataset picked up a row-count/schema assertion, an explicit dedup or merge strategy, and (for green taxi, weather, and traffic advisory) a deliberate rerun that proves idempotency rather than assuming it.
